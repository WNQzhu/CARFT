...
## that is we have changed the `not_num_mask` function:
...
class MyCLTrainer(Trainer):
                ...
                for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                    ...
                    for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                        with accelerator.accumulate(model):
                            ...
                            base_emb_mask = self.not_num_mask(
                                cur_base_token_seq_list,
                                cur_base_prefix_len_list,
                                aug=True
                            )
                            base_emb_mask = base_emb_mask.to(device)
                            ...
                            model_emb_mask = self.not_num_mask(
                                cur_model_token_seq_list,
                                cur_model_prefix_len_list                                
                            )
                            
                            model_emb_mask = model_emb_mask.to(device)
                            # print("vpreds:", vpreds)
    ...
    def not_num_mask(self, cur_token_seq_list, prefix_len_list, aug=False):
