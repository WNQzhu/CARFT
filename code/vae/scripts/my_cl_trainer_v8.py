# wnq::cl_trainer_v8 (from cl_trainer_v7)
## neg part only using logits for softmax:
### (2) neg: compute the common sub seq; and make the common part as the positive and  others as neg.
#
...
class MyCLTrainer(Trainer):
    ...
    def rollout(self, args, new_batch, text_batch, generation_config):
        ...
        return model_input_ids, model_attention_mask, mask, rew, score_rew, kl_rew, ret, correctness, val, old_logprob, ref_logprob, adv, \
            base_input_ids, base_attention_mask, \
                base_lm_logits, base_hidden_emb
    ...
    def train(self):
        ...
        for update in range(1, args.num_total_batches + 1):
            ...
            print("shape:", base_lm_logits.shape,
                  base_hidden_emb.shape)
            # preprocess
            raw_adv = adv
                ...
                for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                    ...
                    for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                        with accelerator.accumulate(model):
                            ...
                            cur_base_lm_logits = base_lm_logits[
                                micro_batch_inds][:,:-1,:].contiguous()
                            ...
                            corr_mask = (torch.Tensor(
                                cur_correctness).unsqueeze(-1)
                                         > 0.5)
                            corr_mask = corr_mask.to(device)

                            ## neg
                            base_idx_seq_list, model_idx_seq_list = \
                                batch_common_sub_seq_idx_only(
                            ...
                            base_common_mask, base_diff_mask, \
                                model_common_mask, model_diff_mask, \
                                neg_loss_mask = self.neg_mask(
                                    base_len,
                                    base_idx_seq_list,
                                    ...
                                    model_idx_seq_list,
                                    cur_model_prefix_len_list)
                            #base_common_mask = base_common_mask.to(device)
                            base_diff_mask = torch.logical_and(
                                base_diff_mask.to(device),
                                cur_base_attention_mask)                            
                            model_common_mask = torch.logical_and(
                                model_common_mask.to(device),
                                cur_model_attention_mask)
                            model_diff_mask = torch.logical_and(
                                model_diff_mask.to(device),
                                cur_model_attention_mask)
                            neg_loss_mask = neg_loss_mask.to(device).unsqueeze(-1)
                            cur_base_lm_logits = torch.gather(
                                input=cur_base_lm_logits,
                                   dim=-1,
                                   index=cur_base_input_ids.unsqueeze(-1)
                            ).squeeze(-1)
                            
                            base_diff_weight = self.masked_softmax(
                                cur_base_lm_logits,
                                base_diff_mask)
                            
                            z_1 = torch.sum(
                                cur_base_hidden_emb *
                                base_diff_weight.unsqueeze(-1), dim=1)
                            
                            
                            model_common_weight = self.masked_softmax(
                                vpreds, model_common_mask)

                            z_2 = torch.sum(
                                pred_hidden_emb *
                                model_common_weight.unsqueeze(-1), dim=1)
                            # z2_all = torch.reshape(allgather(z_2), (-1, 64))

                            
                            model_diff_weight = self.masked_softmax(
                                vpreds, model_diff_mask)
                            
                            z_3 = torch.sum(
                                pred_hidden_emb *
                                model_diff_weight.unsqueeze(-1), dim=1)
                            z3_all = torch.reshape(allgather(z_3), (-1, 64))

                            z_all = torch.concat([z_2, z3_all], dim=0)
                            self.accelerator.print(f"z_all.shape: {z_all.shape}")

                            # corr_mask
                            between_sim = f(self.sim_1(z_1, z_2))
                            all_sim = f(self.sim_2(z_2, z_all))
                            com_mask = torch.logical_and(
                                torch.logical_not(corr_mask),
                                neg_loss_mask)
                            # batch_size
                            ...
                            cl_loss_b = torch.sum(com_mask) + 1e-8
                            cl_loss_neg = torch.sum(cl_loss_a) / cl_loss_b
                            cl_loss = cl_loss_neg
                            ## end neg
                    ...
                    del (
                        ...
                        logprob,
                        cur_base_lm_logits, cur_base_hidden_emb,
                        cur_base_input_ids,
                        cur_base_attention_mask,
                        cur_base_input_ids_cpu,
                        cur_model_input_ids_cpu,
    ...
    def not_num_mask(self, cur_token_seq_list, prefix_len_list):
    ...
    def single_neg_mask(self,
        ...
        pos_mask, neg_mask = [], []
        for i, prefix_len in enumerate(prefix_len_list):
            pos_row_mask = [0] * output_size
            neg_row_mask = [0] * prefix_len + [1] * (output_size - prefix_len)
            for idx in idx_seq_list[i]:
                if aug:
                    ...
                    pos_row_mask[idx] = 1
                neg_row_mask[idx] = 0
            if sum(pos_row_mask) == 0:
                for idx in idx_seq_list[i]:
                    pos_row_mask[idx] = 1
            pos_mask.append(pos_row_mask)
            neg_mask.append(neg_row_mask)
        return pos_mask, neg_mask

    def neg_mask(self,
                 base_len,
                 base_idx_seq_list,
                 base_prefix_len_list,
                 model_len,
                 model_idx_seq_list,
                 model_prefix_len_list):
        base_common_mask, base_diff_mask = self.single_neg_mask(
            base_len, base_idx_seq_list,
            base_prefix_len_list)

        model_common_mask, model_diff_mask = self.single_neg_mask(
            model_len,
            model_idx_seq_list,
            model_prefix_len_list)

        loss_mask = []
        for idx_seq in base_idx_seq_list:
            if len(idx_seq) > 0:
                loss_mask.append(1)
            else:
                loss_mask.append(0)

        return torch.BoolTensor(base_common_mask), \
            torch.BoolTensor(base_diff_mask), \
            torch.BoolTensor(model_common_mask), \
            torch.BoolTensor(model_diff_mask), \
            torch.BoolTensor(loss_mask)
