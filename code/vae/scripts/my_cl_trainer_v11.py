# wnq::cl_trainer_v11(based on cl_trainer_v4)
# this version takes only the positive part of the generated cot
# unlike v2, we extracted all the tokens from the ref and postive generated cot with 1 as the reward
...
class MyCLTrainer(Trainer):
    ...
    def rollout(self, args, new_batch, text_batch, generation_config):
        ...
        with torch.no_grad():
            ...
            lm_logits, _, val, model_hidden_emb = self.model(input_ids=model_input_ids, attention_mask=model_attention_mask)
            
            _, _, base_val, base_hidden_emb = self.model(input_ids=base_input_ids, attention_mask=base_attention_mask)

            #+begin_src update 0.1
            base_input_ids_cpu = base_input_ids.cpu().numpy().tolist()
            model_input_ids_cpu = model_input_ids.cpu().numpy().tolist()
                            
            base_token_seq_list = batch_decode_to_tokens(
                                self.processing_class,
                                base_input_ids_cpu)
            base_prefix_len_list = \
                                text_batch['base_input_prefix_length']            
            model_token_seq_list = batch_decode_to_tokens(
                                self.processing_class,
                                model_input_ids_cpu)
            ...
            ref_weight  = self.masked_softmax(base_val,
                                                    base_emb_mask)        
            z_1 = torch.sum(base_hidden_emb * ref_weight.unsqueeze(-1), dim=1)
            ...
            # print("vpreds:", vpreds)
            pred_weight  = self.masked_softmax(val,
                                                    model_emb_mask)
            # print("vpreds:(after)", vpreds) 
                            
            z_2 = torch.sum(model_hidden_emb * pred_weight.unsqueeze(-1), dim=1)

            sim = F.cosine_similarity(z_1, z_2) * 0.1 + 0.2
            sim_list = sim.to(torch.float).cpu().numpy().tolist()
            # print("corr before: ", correctness)
            for i, v in enumerate(correctness):
                if v == 0.1:
                    correctness[i] = sim_list[i]

            #print("corr after: ", correctness)
            
            del _, base_input_ids_cpu, model_input_ids_cpu, \
                model_hidden_emb, base_token_seq_list, model_token_seq_list, \
